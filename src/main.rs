...
impl LinkedList<i32> {
    ...
    /// ```
    fn remove_first_ocurrence(&mut self , value : i32) -> Result<i32 , String>{
    ...
    fn search(&self, search_value: i32) -> Result<&NodeLink<i32>, bool> {
        if self.empty() {
            Err(false)
        } else {
    ...
    fn see_list(&self) -> Result<String, String> {
